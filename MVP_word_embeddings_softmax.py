...
class EmbModel:
    def __init__(self, vocab_size, dim, graph, batch_size, logdir):
        ...
        # target_one_hot = tf.one_hot(self.target_placeholder, dim)
        # context_one_hot = tf.one_hot(self.context_placeholder,dim)
        # h_target = tf.matmul(target_one_hot, self.rho_embeddings)
        h_target = tf.nn.embedding_lookup(self.rho_embeddings, self.target_placeholder)
        logits = tf.matmul(tf.reshape(h_target, [batch_size, 1, dim]),
                           [tf.transpose(self.alpha_embeddings)] * batch_size)
        probs = tf.nn.softmax(logits)
        variable_summaries(logits, "logits")
        variable_summaries(probs, "probs")
        # self.loss = -tf.reduce_sum(context_one_hot * tf.log(probs))
